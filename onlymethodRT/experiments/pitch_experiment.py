...
def analyze_and_plot_pitch():
    ...
    for task in tasks:
        ...
        if len(indices) == 0:
            ...
            in_task = pitches[indices]
            inside = (in_task >= task['low']) & (in_task <= task['high'])
            accuracy = np.sum(inside) / len(inside) * 100
            in_range = accuracy == 100.0

        task_results.append((task, in_range, accuracy))
        if in_range:
            successful += 1

    print(f"\n✅ Успешных попаданий в нужную частоту: {successful} из {total}\n")

    for i, (task, success, accuracy) in enumerate(task_results, 1):
        status = "✅" if success else "❌"
        print(f"{status} Задание {i}: точность попадания — {accuracy:.1f}%")
