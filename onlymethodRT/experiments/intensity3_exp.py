...
def generate_tasks():
    ...
    while t + TASK_DURATION <= GAME_DURATION:
        ...
        tasks.append({'start': t, 'end': t + TASK_DURATION, 'low': low, 'high': high, 'center': center_db})
        t += TASK_INTERVAL
    return tasks
...
def analyze_and_plot_voice_intensity():
    ...
    for idx, task in enumerate(tasks, 1):
        ...
        in_range_mask = (intensities[indices] >= low) & (intensities[indices] <= high)
        in_range_time = np.sum(in_range_mask) * (BLOCKSIZE / SAMPLERATE)
        in_range_pct = (in_range_time / total_time) * 100 if total_time > 0 else 0

        # Доп. маски
        quiet_mask = intensities[indices] < low
        normal_mask = (intensities[indices] >= low) & (intensities[indices] <= high)
        loud_mask = intensities[indices] > high

        quiet_time = np.sum(quiet_mask) * (BLOCKSIZE / SAMPLERATE)
        normal_time = np.sum(normal_mask) * (BLOCKSIZE / SAMPLERATE)
        loud_time = np.sum(loud_mask) * (BLOCKSIZE / SAMPLERATE)

        quiet_pct = (quiet_time / total_time) * 100
        normal_pct = (normal_time / total_time) * 100
        loud_pct = (loud_time / total_time) * 100

        # Итоговая классификация
        dominant_zone = max(
            [("Тихо", quiet_time), ("Нормально", normal_time), ("Громко", loud_time)],
            key=lambda x: x[1]
        )[0]

        task_results.append((task, in_range_pct >= 100))

        print(f"Задание {idx} ({start:.1f}–{end:.1f} с): цель = {task['center']:.1f} дБ ±{TOLERANCE_DB}")
        print(f"- Попадание в диапазон: {in_range_time:.2f} сек ({in_range_pct:.1f}%)")
        print(f"- Временная маска:")
        print(f"  • Тихо:     {quiet_time:.2f} сек ({quiet_pct:.1f}%)")
        print(f"  • Нормально: {normal_time:.2f} сек ({normal_pct:.1f}%)")
        print(f"  • Громко:   {loud_time:.2f} сек ({loud_pct:.1f}%)")
...
def main():
    print("🔊 Запуск анализа интенсивности с расширенной оценкой...")
