...
SCROLL_SPEED = 25
low_quiet, high_quiet = 40, 46
low_norm, high_norm = 45, 51
low_loud, high_loud = 60 ,66
...
class Artifact(arcade.Sprite):
    def __init__(self, intensity):
        ...
        if intensity in range (low_norm, high_norm):
            self.volume_label = "Нормально"
        elif intensity in range (low_loud, high_loud):
            self.volume_label = "Громко"
        elif intensity in range (low_quiet, high_quiet):
...
class VoiceArcadeGame(arcade.Window):
    ...
    def spawn_artifacts(self):
        ...
        if current_time - self.last_artifact_time > 6:
            intensity_range = random.choice([(low_norm, high_norm-1), (low_loud, high_loud-1)])
