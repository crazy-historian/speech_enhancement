...
class GameConfigWindow(QWidget):
    ...
    def start_game(self):
        ...
        global WAVE_INTERVAL, WAVE_SIZE, GAME_DURATION
...
class VoiceArcadeGame(arcade.Window):
    ...
    def on_update(self, delta_time):
        ...
        if self.wave_in_progress and self.wave_collected == self.wave_size:
        ...
        if self.wave_in_progress:
            if self.gems_spawned < WAVE_SIZE:
                now = time.time()
                if now - self.last_gem_time >= COLLECTION_TIME / WAVE_SIZE:
                    self.spawn_gem()
                    self.last_gem_time = now
                    self.gems_spawned += 1
            else:
                if time.time() - self.last_gem_time >= WAVE_INTERVAL:
                    self.start_new_wave()
    ...
    def start_new_wave(self):
        ...
        self.last_gem_time = self.last_wave_time
