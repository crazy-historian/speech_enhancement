...
class LetterArtifact(arcade.Sprite):
    def __init__(self, letter, intensity, offset_x=0):
        texture_path = f"alphabet/{letter} буква.png"
        super().__init__(texture_path, scale=0.125)
        self.true_x = SCREEN_WIDTH + 100 + offset_x
        self.center_x = round(self.true_x)

        normalized = (intensity - INTENSITY_MIN) / (INTENSITY_MAX - INTENSITY_MIN)
        normalized = max(0.0, min(normalized, 1.0))
        self.center_y = GROUND_Y + normalized * (AIR_Y - GROUND_Y)
        self.collected = False

    def update(self, delta_time=1/200):
        self.true_x -= SCROLL_SPEED * delta_time
        self.center_x = round(self.true_x)
        if self.right < 0:
            self.remove_from_sprite_lists()

    def draw(self):
        draw_x = round(self.center_x)
        draw_y = round(self.center_y)
        arcade.draw_rectangle_filled(
            draw_x, draw_y,
            self.width + 10, self.height + 10,
            color=arcade.make_transparent_color(arcade.color.WHITE, 180)
        )
        old_x, old_y = self.center_x, self.center_y
        self.center_x, self.center_y = draw_x, draw_y
        super().draw()
...
class ArtifactGroup:
    def __init__(self, task, intensity):
        ...
        offset = 0
        for letter in task:
            artifact = LetterArtifact(letter, intensity, offset_x=offset)
            self.letters.append(artifact)
            offset += 70
        self.collected = False
    ...
    def update_and_check(self, player, delta_time):
        ...
        for letter in self.letters:
            ...
            if not letter.collected:
                if letter.check_collision(player):
                    continue
                all_collected = False
        if all_collected:
            self.collected = True
...
class Artifact(arcade.Sprite):
    def __init__(self, intensity):
        super().__init__(TEXTURE_ONE, scale=0.1)
        self.center_x = SCREEN_WIDTH + 100
        normalized = (intensity - INTENSITY_MIN) / (INTENSITY_MAX - INTENSITY_MIN)
        normalized = max(0.0, min(normalized, 1.0))  # защита от выхода за пределы
        self.center_y = GROUND_Y + normalized * (AIR_Y - GROUND_Y)
    ...
    def check_collision(self, player):
        center_distance_x = abs(self.center_x - player.center_x)
        center_distance_y = abs(self.center_y - player.center_y)

        # Порог допустимого отклонения — можно настроить
        max_offset_x = 40  # пикселей
        max_offset_y=40

        if center_distance_x <= max_offset_x and center_distance_y <= max_offset_y:
            self.collected = True
            self.remove_from_sprite_lists()
            return True
