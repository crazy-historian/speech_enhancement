...
class TextInputDialog(QDialog):
    def __init__(self, title: str, label: str, initial: str = "", parent=None):
        ...
        self.name_edit = QLineEdit(self); self.name_edit.setText(initial)
...
class ProfileSelectWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Выбор профиля (VoiceCar)")
    ...
    def delete_selected(self):
        name = self._current_name()
        if not name:
            QMessageBox.information(self, "Удаление", "Выберите профиль."); return
    ...
    def open_selected_profile(self):
        name = self._current_name()
        if not name:
            QMessageBox.information(self, "Открыть", "Выберите профиль."); return
        self.hide()
