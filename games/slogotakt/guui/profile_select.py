from PyQt6.QtWidgets import (
    QWidget, QVBoxLayout, QPushButton, QListWidget, QMessageBox, QLineEdit, QDialog, QDialogButtonBox, QLabel, QHBoxLayout
)
...
class ProfileSelectWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Выбор профиля (slogotakt)")
        ...
        for b in (btn_create, btn_rename, btn_delete, btn_open):
            btns.addWidget(b)
    ...
    def rename_selected(self):
        ...
        if not name:
            QMessageBox.information(self, "Переименование", "Выберите профиль."); return
    ...
    def delete_selected(self):
        name = self._current_name()
        if not name:
            QMessageBox.information(self, "Удаление профиля", "Выберите профиль."); return
        reply = QMessageBox.question(
            self, "Подтверждение", f"Точно удалить профиль «{name}»?",
