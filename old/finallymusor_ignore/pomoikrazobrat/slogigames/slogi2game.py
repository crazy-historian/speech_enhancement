...
class Gem(arcade.Sprite):
    ...
    def __init__(self, x, y, word):
        ...
        for i, letter in enumerate(word):
            letter_sprite = arcade.Sprite(os.path.join(ALPHABET_FOLDER, f"{letter} beresta.jpg"), scale=0.5)
...
class VoiceArcadeGame(arcade.Window):
    ...
    def start_new_wave(self):
        ...
        # **Расчёт расстояния между первым и последним камнем**
        total_distance = COLLECTION_TIME * SCROLL_SPEED  # D = n * speed
        spacing_x = total_distance / (self.wave_size - 1)  # Равномерное расстояние
        # **Настройка равномерных интервалов**
        start_x = SCREEN_WIDTH + 100  # Начальная точка появления гемов (за экраном)
        # Добавляем первый камень сразу
        # self.add_gem()

    # Планируем добавление остальных камней
        arcade.schedule(self.add_gem, COLLECTION_TIME / WAVE_SIZE)
        # fixed_y = AIR_Y  # Все камни на одной высоте
        # gem_x = start_x   # Равномерное размещение по X
        # new_gem = Gem(gem_x, fixed_y)  # Передаём координаты в `__init__`
        # self.gem_list.append(new_gem)
        # for _ in range(WAVE_SIZE-1):
        #     if time.time() - self.gem_wave_time >= COLLECTION_TIME/WAVE_SIZE:
        #         gem_x = start_x   # Равномерное размещение по X
        #         new_gem = Gem(gem_x, fixed_y)  # Передаём координаты в `__init__`
        #         self.gem_list.append(new_gem)
        #         self.gem_wave_time = time.time()
    ...
    def add_gem(self, delta_time=0):
        ...
        if self.gem_count < self.wave_size:
            ...
            fixed_y = AIR_Y
            new_gem = Gem(gem_x, fixed_y, self.current_task)
            self.gem_list.append(new_gem)
